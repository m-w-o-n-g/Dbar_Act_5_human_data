...
for frame = all_frames
    ...
    size_Vmulti_perf = size(size(Vmulti_perf));

    min_col_volt = Inf;
    min_frame = 0;
    for i = 1:size((Vmulti_perf), 3)
        col_voltage = normalize(Vmulti_perf(:,13,i));
        if col_voltage < min_col_volt
            min_col_volt = col_voltage;
            min_frame = i;
        end
    end
    ...
    % Vref = mean(Vmulti_perf, 3);
    Vref = Vmulti_perf(:,:,min_frame);
