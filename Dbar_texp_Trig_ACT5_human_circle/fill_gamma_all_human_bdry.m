...
output_directory = [];
outdir = 'gammas/';
out_fname = 'feb5_106_gamma_cond_distributions_Sbj002_vent_set.mat';
...
for frame = all_frames 
    ...
    if display_images_to_screen == 1
        for jj = frames_to_plot
            ...
            title(['Frame number = ', num2str(frame), ', min trunc = ', num2str(init_trunc), ', max trunc = ', num2str(max_trunc), ', refframe = ', num2str(ref_frame)])
        end
    end
    ...
    if( save_dbar_output_as_mat_file == 1)
        save([output_directory, data_fname, '_GenDom_Skip2Trig', num2str(refimg), '_texp_R', '_',  num2str(max_trunc), '_M', num2str(M),'_zstep', num2str(hh), '.mat'], 'gam_real', 'max_trunc', 'M', 'hh', 'xx', 'numz' );
    end
