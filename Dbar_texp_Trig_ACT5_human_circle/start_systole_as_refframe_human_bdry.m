...
output_directory = [];
outdir = 'gammas/';
out_fname = 'feb5_133_gamma_cond_distributions_Sbj002_perf_chunk.mat';
...
num_refframes = length(start_systoles);
...
for cycle_idx = 1:num_refframes
    curr_start_systole = start_systoles(cycle_idx);

    refframe = start_systoles(cycle_idx);  % use the i-th start_systole as the current iteration's reference frame.
    ...
    if cycle_idx == 1
        ...
        start_window = round((start_systoles(cycle_idx-1) + curr_start_systole) / 2);
    end

    if cycle_idx == num_refframes
        end_window = endframe;
    else
        end_window = round((curr_start_systole + start_systoles(cycle_idx+1)) / 2);
    end
    ...
    for frame = curr_frames
        ...
        for jj = 1:num_frames
            ...
            disp("frame number in the current cycle: " + actual_frame)
        ...
        end % finished reconstructing 1 frame.
        ...
        % save only the real part of gamma. gamma_real is same size as gamma.
        ...
        outstr = [outdir, '/', datafname, '_R', num2str(init_trunc),'_',  num2str(max_trunc), '_M', num2str(M)]; 
        ...
        if display_images_to_screen == 1
            for jj = frames_to_plot
                ...
                % imagesc(xx,xx,rot90(squeeze(gamma_real(jj,:,:))),[datamin, datamax]);
                ...
                title(['Frame number = ', num2str(frame), ', trunc radius = ', num2str(max_trunc), ', refframe = ', num2str(ref_frame)])
        ...
        if( save_dbar_output_as_mat_file == 1)
            save([output_directory, data_fname, '_GenDom_Skip2Trig', num2str(refimg), '_texp_R', '_',  num2str(max_trunc), '_M', num2str(M),'_zstep', num2str(hz), '.mat'], 'gamma_real', 'max_trunc', 'M', 'hz', 'xx', 'numz' );
        end
        
        fclose('all');

    end % finished reconstructing all frames within the cycle.
    disp("All images within the cycle have now been processed")
...
if save_gamma_real_as_mat_file == 1
