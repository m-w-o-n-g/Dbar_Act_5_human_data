...
% This is for SIMUlATED DATA using TRIG PATTERNS on a GENERAL DOMAIN.
%
...
Non_noisy = load('Voltages/Ellipse_1p0_realistic_inclusion_CPamp0p35_back0p3_conds0p5_0p2_0p2_z2p4_voltages.mat');
Noisy = load('Voltages/Ellipse_1p0_noiselevel_0p001_run1_voltages');
V = Noisy.V_noisy;
J = Non_noisy.currents;
total_num_frames = 1;

% Load homogenous data
load('Voltages/Ellipse_ratio_1p0_clb.mat')
Vref = voltages;
...
% Specify a noncircular truncation region for the low-pass Fourier domain filter
init_trunc = 3.5;        % Initial trunc. radius. Choose something smallish
...
for jj = 1:num_frames
    ...
    max_real_texp = max(abs(real(texp(kidx_init))));
    max_imag_texp = max(abs(imag(texp(kidx_init))));
...
if( save_dbar_output_as_mat_file == 1)
    save([output_directory, dataset_name, '_GenDom_Skip2Trig', num2str(refimg), '_texp_R', num2str(init_trunc),'_',  num2str(max_trunc), '_M', num2str(M),'_zstep', num2str(hh), '.mat'],...
        'gam_real', 'init_trunc', 'max_trunc', 'M', 'hh', 'xx', 'numz' );
end
