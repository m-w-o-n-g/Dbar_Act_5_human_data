...
datafname = 'chunk_Sbj001_93kHz_perf_24_10_15_11_05_09_1';
...
% extract mid-diastole. These will be used as reference frames.
mid_diastoles = find_mid_diastoles(Vmulti_perf);
disp(mid_diastoles) % there are 26.

% "grab" all selected frames in the dataset.
all_frames = startframe:endframe; 

% remove the mid-diastole frames from the list of frames to reconstruct.
frames_to_reconstruct = setdiff(all_frames, mid_diastoles);
...
num_refframes = length(mid_diastoles);



% % COMMENT/UNCOMMENT: for using every other mid-diastole as sliding window reference frame.
% reference_indices = 1:2:num_refframes;
% mid_diastoles = mid_diastoles(reference_indices);
% num_refframes = length(mid_diastoles);
...
for cycle_idx = 1:num_refframes
    curr_mid_diastole = mid_diastoles(cycle_idx);
    % disp("Current mid-diastole: " + curr_mid_diastole)
    
    refframe = mid_diastoles(cycle_idx);  % use the i-th mid-diastole as the current iteration's reference frame.
    % disp("Reference frame #: " + cycle_idx)
    % disp("Reference frame: " + refframe)
    ...
    %==========================================================================================
    % % COMMENT/UNCOMMENT for letting the subset of frames to reconstruct against the current iteration's reference frame = all the frames that fall between midpt of prev->cur reference frame and midpt of cur->next reference frame..
    % if cycle_idx == 1
    %     start_window = startframe;
    % else
    %     start_window = round((mid_diastoles(cycle_idx-1) + curr_mid_diastole) / 2);
    % end
    % 
    % if cycle_idx == num_refframes
    %     end_window = endframe;
    % else
    %     end_window = round((curr_mid_diastole + mid_diastoles(cycle_idx+1)) / 2);
    % end
    % 
    ...
    %==========================================================================================
    % COMMENT/UNCOMMENT for letting the subset of frame to reconstruct against the current iteration's reference frame = all the frames after the reference frame --> the next reference frame (inclusive). 
    if cycle_idx == 1
        start_window = startframe;
    else
        start_window = curr_mid_diastole + 1; % diff imaging = we don't want to reconstruct the reference frame.
    end

    if cycle_idx == num_refframes
        end_window = endframe;
    else
        end_window = mid_diastoles(cycle_idx+1); % grab the next mid-diastole.
    end
    ...
    % % = all the frames that fall between previous reference frame and next reference frame. 
    % curr_frames = frames_to_reconstruct( ...
    %     frames_to_reconstruct > mid_diastoles(max(cycle_idx-1,1)) & ...
    %     frames_to_reconstruct < mid_diastoles(min(cycle_idx+1, end)) );

    % % reconstruct frames around the current mid-diastole: 5 before mid-diastole --> 5 after mid-diastole
    % % disp("all frames: " + all_frames)
    % % disp("startframe: " + startframe)
    % % disp("curr mid diastole: " + curr_mid_diastole)
    % frames_to_reconstruct = find(all_frames >= startframe & all_frames <= curr_mid_diastole + 5);
    % disp("number of frames to reconstruct: " + length(frames_to_reconstruct))
    % 
    % % increment the starting frame for next iteration
    % startframe = curr_mid_diastole + 5;
...
if save_movie == 1
    ...
    movie_outFname = ['jan26_1309_short_Dbar_movie_refframe=middiastole_dataset_09_1', datafname];         % output filename for recon movie.
