function srii=gen_int_sr_th(theta,a,M,ii)
...
%---------Output--------------------
% srii computes s(theta)r^ii(theta) for the given boudary information
r_th=a(1)*ones(size(theta));
r_prime=zeros(size(theta));
for m=1:M
    r_th=r_th+a(m+1)*cos(m*theta)+a(m+M+1)*sin(m*theta);
    r_prime=r_prime+m*a(m+M+1)*cos(m*theta)+m*a(m+1)*sin(m*theta);
end
s_th=sqrt(r_th.^2+r_prime.^2);
srii=s_th.*r_th.^ii;
% polar(theta,r_th)
