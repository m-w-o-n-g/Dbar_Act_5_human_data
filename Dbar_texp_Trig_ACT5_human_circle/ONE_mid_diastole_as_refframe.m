% using just one of the mid-diastoles as the reference frame for all frame reconstructions (NO cycle iterating).
...
disp("number of reference frames (mid diastoles): " + num_refframes)


refframe = mid_diastoles(5);
curr_frames = startframe:endframe;
curr_frames(curr_frames == refframe) = [];
num_frames = numel(curr_frames);
...
% all the frames that fall between midpt of prev->cur reference frame and midpt of cur->next reference frame.
% if cycle_idx == 1
...
for jj = 1:num_frames
    disp("frame being reconstructed: " + num2str(jj))
    % disp("iteration: " + jj)
    % V = Vmulti_perf(:,:,jj); 
...
if(display_images_to_screen == 1 || save_images_as_jpg_files == 1 )
    for jj = frames_to_plot
        ...
        imagesc(xx,xx,flipud(squeeze(gamma_all(:,:,frame_for_gamma_all))),[datamin, datamax]);
